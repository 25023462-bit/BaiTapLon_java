...
public class AuctionDetailController implements Initializable {
    ...
    private void loadExistingBidHistory() {
        ...
        for (BidTransactionInfo tx : history) {
            bidHistory.add(tx.getAuctionName() + " đặt $" + String.format("%,.2f", tx.getAmount()));
            if (priceSeries != null) {
    ...
    private void handleServerMessage(Object obj) {
        ...
        switch (msg.getType()) {
            ...
            case AUTO_BID_SUCCESS -> {
                ...
                if (autoBidStatusLabel != null)
                    autoBidStatusLabel.setText("Auto-bid kích hoạt đến $" + String.format("%,.2f", msg.getAmount()));
        ...
        if (connected && out != null) {
            new Thread(() -> {
                try {
                    ...
                    Platform.runLater(() -> {
                        bidHistory.add(0, String.format("Bạn đặt $%,.2f", amount));
            ...
            bidAmountField.clear();
            showBidResult(String.format("Đặt giá: $%,.2f (offline)", amount), true);
        }
    }
    ...
    private void handleRegisterAutoBid() {
        ...
        if (maxBid <= currentPrice) {
            if (autoBidStatusLabel != null)
                autoBidStatusLabel.setText("MaxBid phải lớn hơn giá hiện tại $" + String.format("%,.2f", currentPrice));
            return;
        }
    ...
    private void updatePrice(double price, String leaderId) {
        ...
        bidHistory.add(0, (leaderId != null ? leaderId : "?") + " đặt $" + String.format("%,.2f", price));
        if (auction != null) auction.setCurrentPrice("$" + String.format("%,.2f", price));
        addChartPoint(price);
        updateRatingVisibility();
    }
